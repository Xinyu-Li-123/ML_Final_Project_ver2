...
def prepare_img(img_path, target_shape, device, type="rgb"):
    ...
    if type == "rgb":
        img = transform(img).to(device).unsqueeze(0)    # ndarray -> tensor
        return img
    elif type == "yiq":
        luminance_img = y2rgb(luminance_img)        # y channel -> rgb colorspace        
        luminance_img = luminance_img.astype(np.float32)
        luminance_img = transform(luminance_img).to(device).unsqueeze(0)    # ndarray -> tensor
...
def prepare_model(model, device, pretrained=True, pooling_method='max'):
    ...
    if model == 'vgg16':
        if experimental:
            ...
            model = Vgg16Experimental(requires_grad=False, show_progress=True, pretrained=pretrained, pooling_method=pooling_method)
